...
def show_tab_trend():
  ...
  with map_col1:
    with st.container(border=True):
      ...
      with select_col1:
        map_selected_ym = st.selectbox('Select Month', map_ym_options, index=None, key='map1', placeholder="Select Month")
      with select_col2:
        map_selected_type = st.selectbox('Select Type', map_type_options, index=None, key='map2', placeholder="Select Type")
